...
export default function SignUp() {
  ...
  const { handleSubmit, setValue, control, watch, formState: { isDirty, isValid, errors, isSubmitting }, setError } = useForm<FormValues>({
    mode: 'all',
    defaultValues: {
      login: '',
      password: '',
  ...
  const onSubmit = async (data: FormValues, event?: BaseSyntheticEvent) => {
    ...
    (['password', 'login', 'passwordConfirmation'] as const).forEach((field) => setValue(field, ''))
  }
  ...
  return (
    <PageContainer>
      <h1 className={styles.header}>Hello, nice to meet you!</h1>
        ...
        <p className={styles.linkParagraph}>Already have an account? <Link className={styles.link} href="/login">Log in</Link></p>
    </PageContainer>
